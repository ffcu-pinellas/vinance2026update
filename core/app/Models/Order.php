...
class Order extends Model
{
    ...
    public function orderTypeBadge(): Attribute
    {
        return new Attribute(function () {
            ...
            } elseif ($this->order_type == Status::ORDER_TYPE_MARKET) {
                $html = '<span>' . trans('Market') . '</span>';
            } else {
                $html = '<span>' . trans('Stop Limit') . '</span>';
            }
            return $html;
        });
    }
...
{
    return $query->where('orders.status', Status::ORDER_OPEN);
}
public function scopePending($query)
{
    return $query->where('orders.status', Status::ORDER_PENDING);
}
public function scopeCompleted($query)
{
    return $query->where('orders.status', Status::ORDER_COMPLETED);
}
public function scopeCanceled($query)
{
    return $query->where('orders.status', Status::ORDER_CANCELED);
}
