...
class AdminController extends Controller
{
 ...
 // Add this line for active stakes count
    $activeStakesCount = \App\Models\Stake::where('status', 'active')->count();
    view()->share('activeStakesCount', $activeStakesCount);
    ...
    public function requestReport()
    {
        ...
        if (!$response || !@$response->status || !@$response->message) {
            return to_route('admin.dashboard')->withErrors('Something went wrong 1');
        }
    ...
    public function reportSubmit(Request $request)
    {
        ...
        $response             = json_decode($response);
        if (!$response || !@$response->status || !@$response->message) {
            return to_route('admin.dashboard')->withErrors('Something went wrong 2');
        }
        if ($response->status == 'error') {
