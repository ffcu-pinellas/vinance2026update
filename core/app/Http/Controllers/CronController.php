...
use App\Services\StakingRewardService;
use Carbon\Carbon;
use Exception;
...
class CronController extends Controller
{
    ...
    public function incompleteBinary()
    {
        try {
            ...
            foreach ($incompleteTrades as $trade) {
                ...
                $time  = (int) (floor($time / 60) * 60) * 1000;
    ...
    private function refundBinaryTrade($trade)
    {
     ...
     public function processStakingRewards()
    {
       // if (!request()->hasValidSignature()) {
          //  abort(401);
       // }

        $stakingRewardService = new StakingRewardService();
        $stakingRewardService->processRewards();

        return response()->json(['status' => 'success', 'message' => 'Staking rewards processed successfully']);
    }
