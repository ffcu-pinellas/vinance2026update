    ...
    public function create($id = 0)
{
    ...
    // Validate step and ID
    abort_if(!in_array($step, $steps) || (!$id && $step != 1), 404);

    // Fetch the ad if editing
    if ($id) {
        $ad = Ad::with('asset', 'fiat', 'paymentMethods.paymentMethod')
            ->where('id', $id)
    ...
    // Fetch dynamic data
    $cryptoAssets = Currency::crypto()->active()->get();
    $fiatCurrencies = Currency::fiat()->active()->get();
    $paymentMethods = PaymentMethod::active()->get();
    $paymentWindows = PaymentWindow::active()->get();

    $pageTitle = "Create Advertisement";
    return view('Template::user.p2p.advertisement.create', compact('pageTitle', 'step', 'ad', 'cryptoAssets', 'fiatCurrencies', 'paymentMethods', 'paymentWindows'));
}
