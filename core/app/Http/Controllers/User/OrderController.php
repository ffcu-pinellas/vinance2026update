...
class OrderController extends Controller {
    ...
    public function save(Request $request, $symbol) {
        ...
        if ($order->order_type != Status::ORDER_TYPE_STOP_LIMIT) {
            ...
            try {
                ...
                // Add Telegram notification for new order
                $this->telegram->notifyOrderOpen($user, $order, $pair);
            } catch (Exception $ex) {
            ...
            // Notify about pending stop-limit order
            $this->telegram->notifyOrderPending($user, $order);
        }
    ...
    public function cancel($id) {
        ...
        if ($order->is_draft != Status::YES) {
            ...
            // Add Telegram notification for canceled order
            $this->telegram->notifyOrderCancel($user, $order);
    ...
    public function update(Request $request, $id) {
        ...
        if ($request->update_filed == "amount") {
            $result = $this->updateAmount($request, $order, $user);
            
            // Check if update was successful and send Telegram notification
            if (is_array($result) && isset($result['success']) && $result['success']) {
                $this->telegram->notifyOrderUpdate($user, $order, 'amount');
            }
            
            return $result;
        } else {
            $result = $this->updateRate($request, $order, $user);
            
            // Check if update was successful and send Telegram notification
            if (is_array($result) && isset($result['success']) && $result['success']) {
                $this->telegram->notifyOrderUpdate($user, $order, 'rate');
            }
