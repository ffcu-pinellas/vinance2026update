...
class TradeManager
{
    ...
    public function trade()
    {
        ...
        foreach ($buySideOrders as $buySideOrder) {
            ...
            // Determine the timeframe based on market liquidity
            $liquidityState = $this->isHighLiquidity($pairId);
            $timeframe = $liquidityState === true ? 10 : ($liquidityState === false ? 120 : 30);
            \Log::info('Market Liquidity State:', [
                'pair_id' => $pairId,
                'liquidity' => $liquidityState === true ? 'High' : ($liquidityState === false ? 'Low' : 'Neutral'),
                'timeframe' => $timeframe,
            ]);

            // Fetch sell orders within the dynamic timeframe
            $sellSideOrders = Order::with('pair', 'user')
                ...
                ->sellSideOrder()
                ->open()
                ->where('created_at', '>=', now()->subMinutes($timeframe))
                ->orderBy('id', 'ASC')
                ->get();
            ...
            foreach ($sellSideOrders as $sellSideOrder) {
                ...
                $tradeAmount = min($sellingAmount, $buyAmount);

                $this->createTrade($tradeSideBuy, $buySideOrder, $rate, $tradeAmount, $buySideOrder->user_id);

                $buyerWallet = Wallet::where('user_id', $buySideOrder->user_id)
                    ->where('currency_id', $buySideOrder->pair->coin->id)
                    ->spot()
                    ->first();
                ...
                $sellerWallet = Wallet::where('user_id', $sellSideOrder->user_id)
                    ->where('currency_id', $sellSideOrder->pair->market->currency_id)
                    ->spot()
                    ->first();
    ...
    private function isHighLiquidity($pairId)
    {
        $highLiquidityThreshold = config('trading.high_liquidity_threshold', 100);
        $lowLiquidityThreshold = config('trading.low_liquidity_threshold', 20);

        $orderCount = Order::where('pair_id', $pairId)
            ->where('created_at', '>=', now()->subMinutes(10))
            ->count();

        if ($orderCount >= $highLiquidityThreshold) {
            return true;
        } elseif ($orderCount <= $lowLiquidityThreshold) {
            return false;
        } else {
            return null;
        }
